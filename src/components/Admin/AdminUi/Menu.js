...
export default function Menu() {
    ...
    const [isDheshboard, setDheshboard] = useState(true);
    return <>
                ...
                <nav aria-label="Sidebar Navigation" className="peer-checked:w-64 left-0 z-10 flex h-screen w-0 flex-col overflow-hidden bg-gray-700 text-white transition-all md:h-screen md:w-64 lg:w-72">
                    ...
                    <ul className="mt-8 space-y-3 md:mt-20">
                        <li className="relative">
                            <button onClick={() => { setDheshboard(false); setOrders(true) }} className="focus:bg-slate-600 hover:bg-slate-600 flex w-full space-x-2 rounded-md px-10 py-4 text-gray-300 focus:outline-none">
                                <span
                                ><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
